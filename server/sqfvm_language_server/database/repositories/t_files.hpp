...
namespace sqfvm::lsp::repositories
{
    struct file
    {
        ...
        static file get(sqlite::database& db, std::filesystem::path filepath)
        {
            ...
            if ((res = result_1) != sqlite::result::OK) { return { filepath, 0, {} }; }
            res = stmnt_->bind_text(0, filepath.string());
            if (res != sqlite::result::OK) { return { filepath, 0 }; }
            res = stmnt_->next();
            if (res != sqlite::result::OK) { return { filepath, 0 }; }
        ...
        static sqlite::result set(sqlite::database& db, const file& f)
        {
            ...
            if ((res = stmnt_->bind_int64(1, f.timestamp)) != sqlite::result::OK) { return res; }
            if ((res = stmnt_->bind_int(2, static_cast<int>(f.state))) != sqlite::result::OK) { return res; }
        ...
        static sqlite::result all(sqlite::database& db, std::vector<file>& ref_files)
        {
            ...
            if ((res = result_1) != sqlite::result::OK) { return res; }
