...
namespace sqlite
{
    enum class result
    {
        /* Successful result */
        OK = 0, /* Generic error */
        ERROR = 1, /* Internal logic error in SQLite */
        INTERNAL = 2, /* Access permission denied */
        PERM = 3, /* Callback routine requested an abort */
        ABORT = 4, /* The database file is locked */
        BUSY = 5, /* A table in the database is locked */
        LOCKED = 6, /* A malloc() failed */
        NOMEM = 7, /* Attempt to write a readonly database */
        READONLY = 8, /* Operation terminated by sqlite3_interrupt()*/
        INTERRUPT = 9, /* Some kind of disk I/O error occurred */
        IOERR = 10, /* The database disk image is malformed */
        CORRUPT = 11, /* Unknown opcode in sqlite3_file_control() */
        NOTFOUND = 12, /* Insertion failed because database is full */
        FULL = 13, /* Unable to open the database file */
        CANTOPEN = 14, /* Database lock protocol error */
        PROTOCOL = 15, /* Internal use only */
        EMPTY = 16, /* The database schema changed */
        SCHEMA = 17, /* String or BLOB exceeds size limit */
        TOOBIG = 18, /* Abort due to constraint violation */
        CONSTRAINT = 19, /* Data type mismatch */
        MISMATCH = 20, /* Library used incorrectly */
        MISUSE = 21, /* Uses OS features not supported on host */
        NOLFS = 22, /* Authorization denied */
        AUTH = 23, /* Not used */
        FORMAT = 24, /* 2nd parameter to sqlite3_bind out of range */
        RANGE = 25, /* File opened that is not a database file */
        NOTADB = 26, /* Notifications from sqlite3_log() */
        NOTICE = 27, /* Warnings from sqlite3_log() */
        WARNING = 28, /* sqlite3_step() has another row ready */
        ROW = 100, /* sqlite3_step() has finished executing */
        DONE = 101, /* */
        ...
        CANTOPEN_CONVPATH = (CANTOPEN | (4 << 8)), /* Not Used */
    ...
    enum class type
    {
        INVALID = 0, T_INTEGER = 1, T_FLOAT = 2, T_TEXT = 3, T_BLOB = 4, T_NULL = 5
    };
    ...
    class prepared
    {
        ...
        explicit prepared(void *statement) : m_statement(statement), m_done(false), m_bindable(true)
        {
        }
    ...
    public:
        ...
        {
            m_statement = p.m_statement;
            m_bindable = p.m_bindable;
            m_done = p.m_done;
            p.m_statement = nullptr;
            return *this;
        }
        ...
        [[nodiscard]] size_t parameters() const;

        sqlite::result next();

        [[nodiscard]] size_t columns() const;

        [[nodiscard]] type type(int index) const;
    ...
    public:
        ...
        database(database &&d) noexcept: m_db(d.m_db), m_good(d.m_good), m_path(std::move(d.m_path))
        {
            d.m_db = nullptr;
        }
        ...
        explicit database(const std::filesystem::path &path) : m_db(nullptr), m_good(false)
        {
            open(path);
        }
